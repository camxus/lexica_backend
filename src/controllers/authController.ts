...
import { dynamo, s3, USERS_TABLE } from "../lib/aws";
...
export const signup = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(201).json({
      user: {
        ...userData,
        avatar: userData.avatar && await getSignedImage(s3, userData.avatar),
      },
    });
  } catch (error: any) {
...
export const login = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const userDetailsResponse = await dynamo.send(
      new GetItemCommand({
        TableName: USERS_TABLE,
...
export const getPresignURL = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const signedUrl = await getSignedUrl(s3 as any, command as any, { expiresIn: 300 }); // 5 minutes
...
export const getPresignPOST = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const presignedPost = await createPresignedPost(s3 as any, {
